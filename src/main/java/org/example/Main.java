    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            if (number == 1) {
                ...
                Book book = library.createBook(count++, title, author, LocalDate.parse(year), Book.Status.valueOf(status));
                books.add(book);
            } else if (number == 2) {
                ...
                System.out.println("please enter a status");
                String status = in.next();
                library.updateBook(id, books, title, author, LocalDate.parse(year), Book.Status.valueOf(status));
            } else if (number == 3) {
                System.out.println("please enter a book id ");
                Integer id = in.nextInt();
                library.deleteBook(id, books);
            } else if (number == 4) {
                System.out.println("please enter a book title");
                String title = in.next();
                System.out.println(library.getBooksByTitle(title, books));
            } else if (number == 5) {
                System.out.println("please enter a book author");
                String author = in.next();
                System.out.println(library.getBooksByAuthor(author, books));
            } else if (number == 6) {
                System.out.println(library.sortedByYearPublication(books));
            }
        } while (number != 0);
